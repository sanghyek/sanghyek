...
public int Userupdate(String id,String pw,String email,String phone,String post,String addr1,String addr2) {
	int rowNum=0;
	try{
		conn=Drive();
		stmt=conn.createStatement();//회원정보수정
		String command=null;
		if(pw==null||pw=="") {
		command = "update userinfo set email='"+email+"',phone='"+phone+"',postcode='"+post+"',addr1='"+addr1+"',addr2='"+addr2+"' where id='"+id+"';";
		}else {
		command = "update userinfo set password='"+pw+"',email='"+email+"',phone='"+phone+"',postcode='"+post+"',addr1='"+addr1+"',addr2='"+addr2+"' where id='"+id+"';";
		}
		//System.out.println(command);
